            ...
            <div className="text-center relative">
              ...
              {showProgress && (
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-sm text-cyan-400 mt-3 font-medium">{Math.round(progressValue)}% Complete</p>
                  
                  {/* Real-time Status Message */}
                  <div className="mt-2">
                    <p className="text-xs text-gray-400">
                      {progressValue < 15 ? 'Initializing analysis...' :
                       progressValue < 45 ? 'Processing resume files...' :
                       progressValue < 75 ? 'AI analyzing content...' :
                       progressValue < 95 ? 'Generating insights...' :
              ...
              {showStatusIndicators && (
                <div className="flex justify-center gap-4 mt-6">
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full animate-pulse ${progressValue >= 15 ? 'bg-green-400' : 'bg-gray-400'}`}></div>
                    <span className={`text-xs ${progressValue >= 15 ? 'text-green-400' : 'text-gray-400'}`}>Processing Files</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full animate-pulse ${progressValue >= 45 ? 'bg-green-400' : 'bg-gray-400'}`}></div>
                    <span className={`text-xs ${progressValue >= 45 ? 'text-green-400' : 'text-gray-400'}`}>AI Analysis</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full animate-pulse ${progressValue >= 75 ? 'bg-green-400' : 'bg-gray-400'}`}></div>
                    <span className={`text-xs ${progressValue >= 75 ? 'text-green-400' : 'text-gray-400'}`}>Generating Report</span>
                  </div>
                </div>
              )}
            </div>
