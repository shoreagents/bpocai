...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const countRes = await client.query(`SELECT COUNT(*)::int AS total FROM ai_analysis_results ${whereSql}`, params)
		const total = countRes.rows[0]?.total || 0

		params.push(pageSize, offset)
		const dataRes = await client.query(
			`SELECT ${selectList} FROM ai_analysis_results ${whereSql} ORDER BY created_at DESC LIMIT $${params.length-1} OFFSET $${params.length}`,
			params
		)
		return withCors(NextResponse.json({ page, pageSize, total, items: dataRes.rows }))
	} catch (e) {
		return withCors(NextResponse.json({ error: 'Failed to fetch analysis results' }, { status: 500 }))
