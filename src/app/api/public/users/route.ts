...
export async function GET(request: NextRequest) {
	...
	try {
		...
		const countRes = await client.query(`SELECT COUNT(*)::int AS total FROM users ${whereSql}`, params)
		const total = countRes.rows[0]?.total || 0

		params.push(pageSize, offset)
		const dataRes = await client.query(
			`SELECT ${selectList} FROM users ${whereSql} ORDER BY created_at DESC LIMIT $${params.length-1} OFFSET $${params.length}`,
			params
		)
