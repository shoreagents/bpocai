...
export default function ResetPasswordPage() {
  ...
  useEffect(() => {
    ...
    const access = hash.get('access_token') || qs.get('access_token')
    const refresh = hash.get('refresh_token') || qs.get('refresh_token')

    setAccessToken(access)
    setTokenDetected(!!access)

    supabase.auth.getSession().finally(async () => {
      if (access && refresh) {
              ...
              <CardContent className="space-y-5">
                ...
                {tokenDetected === false && (
                  <div className="text-xs text-yellow-300 bg-yellow-500/10 border border-yellow-500/20 rounded p-2">
                    This page expects a valid password recovery link from your email. If you opened this directly, go back to the login and use “Forgot password?”
                  </div>
                )}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <Input
                        ...
                        className="pl-3 pr-10 h-11 bg-white/5 border-white/20 text-white placeholder:text-gray-400 focus:border-cyan-500 focus:ring-cyan-500/20"
                      />
                      <button
                        type="button"
                ...
                <div className="text-center">
                  <a href="/" className="inline-flex items-center gap-2 text-cyan-400 hover:text-cyan-300 underline text-sm">
